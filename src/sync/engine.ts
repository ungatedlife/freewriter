...
import type { BridgeSettings, Route } from "../settings";
import type { SourceRecord, SyncState } from "../state";
import { countWords, ensureContentPlaceholder, renderTemplate, type DateFormatter, type TemplateContext } from "../template";
...
export class SyncEngine {
	...
	log(level: LogEvent["level"], route: string, message: string): void {
		...
		if (level === "error") console.error(`Bridge${route ? ` [${route}]` : ""}: ${message}`);
	}
	...
	/** Record for the note at this vault path, if Bridge manages it. */
	...
	private pruneState(state: SyncState, settings: BridgeSettings): void {
	...
	private async processEntry(
		...
		const ctx = this.buildContext(route, parsed, entry, folderLabel, sourceKey);
		const template = ensureContentPlaceholder(route.noteTemplate);
		...
		if (!note) {
			...
			if (!report.dryRun) {
				const file = await this.writer.createNote(notePath, renderTemplate(template, ctx, this.formatDate));
				await this.writer.setProperties(file, (fm) => {
					fm[idKey] = sourceKey;
				});
		...
		if (decision === "overwrite") {
			if (!report.dryRun) {
				const newBody = renderTemplate(splitFrontMatter(template).body, ctx, this.formatDate);
				await this.writer.writeNote(note, (split.frontmatter ?? "") + newBody);
		...
		if (!report.dryRun) {
			const copy = await this.writer.createNote(copyPath, renderTemplate(template, ctx, this.formatDate));
	...
	private async handleMissing(route: Route, settings: BridgeSettings, rec: SourceRecord, report: RunReport): Promise<void> {
		if (!report.dryRun) {
			rec.status = "missing";
	...
	private async clearMissing(rec: SourceRecord, note: TFile, settings: BridgeSettings): Promise<void> {
		rec.status = "synced";
		await this.writer.setProperties(note, (fm) => {
			if (fm[settings.statusProperty] === "source-missing") delete fm[settings.statusProperty];
		});
	...
	private buildContext(route: Route, parsed: ParsedDraft, entry: SourceEntry, folderLabel: string, sourceKey: string): TemplateContext {
		return {
			title: parsed.title,
